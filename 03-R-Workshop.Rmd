...
We upload an expression set already available. It corresponds to a subset of samples from a gene expression dataset of breast cancer (BRCA) primary tissue samples from the TCGA project.
...
4. Rename entries in the "sample_type" colunn as "tumor" and "normal"  
...
### Normalization
```{r, eval=T, echo=F}
brca.rle <- brca.rle.log <- brca.filtered
dds <- DESeqDataSetFromMatrix(exprs(brca.filtered), pData(brca.filtered), formula( ~ 1)) # Create DEseq2 specific object
